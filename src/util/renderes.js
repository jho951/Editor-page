...
function drawHandles(ctx, s) {
    ...
    const rot = handles.find((h) => h.code === HANDLE.N);
    if (rot) {
        ctx.beginPath();
...
function drawHandlesHit(ctx, s) {
    ...
    const rot = hs.find((hh) => hh.code === HANDLE.N);
    if (rot) {
        const [r, g, b] = idToRGB(s.id, HANDLE.ROTATE);
        ctx.beginPath();
...
function handlePositions(s) {
    ...
    return [
        { code: HANDLE.NW, x, y },
        { code: HANDLE.N, x: midX, y },
        { code: HANDLE.NE, x: x + w, y },
        { code: HANDLE.E, x: x + w, y: midY },
        { code: HANDLE.SE, x: x + w, y: y + h },
        { code: HANDLE.S, x: midX, y: y + h },
        { code: HANDLE.SW, x, y: y + h },
        { code: HANDLE.W, x, y: midY },
    ];
}
