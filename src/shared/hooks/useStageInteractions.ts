...
import { pickIdAt } from "@canvas/util/picking";
import { screenToWorld, type ViewTransform, type Point as WorldPoint } from "@canvas/util/view";
import { drawEllipsePath, drawPolygonPath, drawStarPath } from "@canvas/util/paths";
import { computeBBox, normalizePath, type Point } from "@canvas/engine/geometry";
import { getDpr } from "@canvas/util/get-dpr";
import { MAX_SCALE, MIN_SCALE } from "@canvas/config/setting";

import type { Shape, HandleKey } from "@canvas/types";
import type { AppDispatch } from '@app/store/store';

export interface UseStageInteractionsParams {
  ovRef: RefObject<HTMLCanvasElement | null>;
  hitRef: RefObject<HTMLCanvasElement | null>;
  ...
  actions: typeof import('../state/canvas.slice').canvasActions;
  beginTextEdit: (shapeId: number) => void;
  isNodeDraggingRef: MutableRefObject<boolean>;
}

interface DragState {
  mode: 'move' | 'resize' | 'create' | 'path-node' | 'pan' | 'zoom';
  startX: number;
  startY: number;
  startWorld: Point;
...
function useStageInteractions(params: UseStageInteractionsParams): void {
    const {
        ovRef,
        hitRef,
        viewRef,
    ...
    useEffect(() => {
        ...
        function toCanvasPt(e: PointerEvent): WorldPoint {
            const rect = ov.getBoundingClientRect();
            const xs =
                ((e.clientX - rect.left) * (ov.width / rect.width)) / getDpr();
            const ys =
                ((e.clientY - rect.top) * (ov.height / rect.height)) / getDpr();
        ...
        function hitHandle(
            ...
            const hit = (r: { x: number; y: number; w: number; h: number }): boolean =>
                pt.x >= r.x &&
                pt.x <= r.x + r.w &&
                pt.y >= r.y &&
                pt.y <= r.y + r.h;
        ...
        function onDbl(e: PointerEvent): void {
        ...
        function onMove(e: PointerEvent): void {
            ...
            if (ds.type === 'pan') {
                const rect = ov.getBoundingClientRect();
                const xs =
                    ((e.clientX - rect.left) * (ov.width / rect.width)) /
                    getDpr();
                const ys =
